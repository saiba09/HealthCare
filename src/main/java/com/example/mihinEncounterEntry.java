...
public class mihin
{
	private static final String BUCKET_NAME = "mihin-data";
	  ...
	  private static final byte[] FAMILY = Bytes.toBytes("ColumnFamily1");
		...
		@Override
            ...
            try {
             Object obj = parser.parse(new FileReader("Encounter_entry.txt"));
             ...
             for (int i = 0; i < 50; i++) {
                JSONObject jsonObject1 = (JSONObject) parser.parse(resource.get(i).toString());
          			...
          			HashMap<String , JSONArray> map2  =  (HashMap<String, JSONArray>) jsonObject1.get("resource");
		 	          ...
		 	          String patientId = patient.substring(patient.indexOf('/')+1);
		 	          JSONObject periodObj  =  (JSONObject) map.get("period");
		 	          String startDate =  (String) periodObj.get("start");
		 	          String endDate =  (String) periodObj.get("end");
		 	          String startTime = startDate.substring(startDate.indexOf('T')+1);
		 	          startDate = startDate.substring(0,startDate.indexOf("T"));
		 	          String endTime = endDate.substring(endDate.indexOf('T')+1);
		 	          endDate = endDate.substring(0,endDate.indexOf("T"));
		          	String kind =  (String) map.get("class");
		 	          String e_id = (String) map.get("id");
		 	          JSONArray indicationObject  =  (JSONArray) map2.get("indication");
		          	int indication_count = indicationObject.size();
		            }
            }
            catch(Exception e){
              e.printStackTrace(); 
	...
	public static void main(String[] args) 
	{
		...
		Pipeline p = Pipeline.create(options);
		 ...
		 p.apply(TextIO.Read.from("gs://mihin-data/formatedEncounterEntry.json")).apply(ParDo.of(MUTATION_TRANSFORM))
